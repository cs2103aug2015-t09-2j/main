...
public class PropertyParser {
    ...
    /*
     * Convert the property string value into appropriate Object based
     * on propertyName. For example, parseProperty(“from”, “24/10/2015”)
     * will return a Date object representing 24/10/2015.
     * 
     * @param propertyName Command property keyword (can be "from", "to", "by",...)
     * @param propertyValue Value of command property inputed by user
     * input value
    ...
    public static void parseProperty(String propertyName, String propertyValue, CommandDetail command){
        switch (propertyName){
            case CommandKeywords.KW_FROM:
                command.setProperty(CommandProperties.TIME_FROM, parseOptionValue(CommandProperties.TIME_FROM, propertyValue));
                break;
            case CommandKeywords.KW_BY:
                command.setProperty(CommandProperties.TIME_BY, parseOptionValue(CommandProperties.TIME_BY, propertyValue));
                break;
            case CommandKeywords.KW_TO:
                command.setProperty(CommandProperties.TIME_TO, parseOptionValue(CommandProperties.TIME_TO, propertyValue));
                break;
            case CommandKeywords.KW_UNTIL:                
                command.setProperty(CommandProperties.TIME_UNTIL, parseOptionValue(CommandProperties.TIME_UNTIL, propertyValue));
                break;
    ...
    /*
     * Convert the option string value into appropriate Object based
     * on optionName.
     * 
     */
